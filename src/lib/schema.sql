...
-- HBOG-LOCAL-SUPABASE-FOUNDATION-R1
-- Keep this legacy schema snapshot aligned with the authoritative migration
-- history under supabase/migrations/. Browser clients may create only pending
-- donations; staff content writes are authorized from public.profiles.
...
update public.events set location = 'Online / Main Sanctuary' where location is null;
...
create policy donations_public_insert
  on public.donations for insert to anon, authenticated
  with check (
    btrim(currency) <> '' and amount > 0 and btrim(frequency) <> ''
    and btrim(payment_method) <> '' and status = 'pending'
  );
...
-- ============================================================
-- HBOG POLICY MANIFEST INTEGRITY R2
-- Keep this bootstrap snapshot aligned with the authoritative migration.
-- ============================================================
